...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 px-4 py-4">
      <nav className="mx-auto max-w-[1600px] overflow-hidden rounded-2xl border border-cyan-500/20 bg-black/55 shadow-[0_0_40px_rgba(34,211,238,.08)] backdrop-blur-xl">
        <div className="flex items-center justify-between border-b border-white/5 px-6 py-2 text-[10px] uppercase tracking-[.35em] text-slate-500">
          ...
          <div className="hidden md:block">PulsePlay Gaming Network</div>
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <NavLink to="/" onClick={() => setOpen(false)} className="group flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black pp-gradient-text">PulsePlay</h1>
              <p className="text-[11px] uppercase tracking-[.45em] text-cyan-400">Gaming Command Center</p>
            </div>
          </NavLink>
            ...
            <div className="grid gap-3">
              {links.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) => `flex items-center gap-3 rounded-xl px-5 py-4 font-bold transition-all ${isActive ? "bg-cyan-400 text-black" : "bg-white/5 text-slate-300 hover:bg-white/10"}`}
                >
