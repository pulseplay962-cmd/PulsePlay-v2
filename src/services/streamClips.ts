...
async function adminFetch(path:string, options:RequestInit={}) {
  ...
  const headers=new Headers(options.headers); headers.set("Content-Type","application/json"); headers.set("Authorization",`Bearer ${session.access_token}`);
  const response=await fetch(`${API_URL}${path}`,{...options,headers}); const data=await response.json();
  if(!response.ok) throw new Error(data?.error||"PulsePlay AI request failed."); return data;
}
...
export async function getStreamVods(sync=true):Promise<StreamVod[]>{const d=await adminFetch(`/api/ai/stream-clips/vods?sync=${sync?"true":"false"}&limit=20`);return d.vods||[]}
export async function getStreamClips(vodId?:string):Promise<StreamClip[]>{const q=vodId?`?vodId=${encodeURIComponent(vodId)}`:"";const d=await adminFetch(`/api/ai/stream-clips/clips${q}`);return d.clips||[]}
